...
//
// Usage (from apps/cli): bun run smoke-tests/lib/compare-replicas.ts <ts-replica-dir> <zig-replica-dir>
//
// Checks:
...
async function main(): Promise<void> {
    ...
    if (!tsReplicaDir || !zigReplicaDir) {
        console.error("Usage: bun run smoke-tests/lib/compare-replicas.ts <ts-replica-dir> <zig-replica-dir>");
        process.exit(2);
    }
