...
export async function removeCommand(context: ICommandContext, assetId: string, options: IRemoveCommandOptions): Promise<void> {
    ...
    const { assetStorage, rawAssetStorage, bsonDatabase, metadataCollection } = await loadDatabase(dbPath, options, uuidGenerator, timestampProvider, sessionId);

    // Remove the asset using the comprehensive removal method
    await removeAsset(assetStorage, rawAssetStorage, sessionId, bsonDatabase, metadataCollection, assetId, true);
