...
export async function addCommand(context: ICommandContext, paths: string[], options: IAddCommandOptions): Promise<void> {
    ...
    const { assetStorage, rawAssetStorage, bsonDatabase, metadataCollection, databaseDir } = await loadDatabase(options.db, options, uuidGenerator, timestampProvider, sessionId);
