...
test_zig_verify_ts_db() {
    ...
    # Damage a file and check both CLIs detect it the same way (a verification that finds problems exits 1).
    ...
    invoke_command "Verify damaged database with TypeScript" "$ts_cli verify --db $db_dir --yes" 1 "ts_damaged_output"
    invoke_command "Verify damaged database with Zig" "$zig_cli verify --db $db_dir --yes" 1 "zig_damaged_output"
