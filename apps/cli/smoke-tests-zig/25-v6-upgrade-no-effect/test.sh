#!/bin/bash
DESCRIPTION="Zig replicate/verify: Test v6 upgrade has no effect"
...
test_v6_database_upgrade_no_effect() {
    ...
    local verify_output
    invoke_command "Verify v6 database after upgrade" "$(get_zig_cli_command) verify --db $temp_v6_dir --yes" 0 "verify_output"
...
test_v6_database_upgrade_no_effect "${1:-25}"
