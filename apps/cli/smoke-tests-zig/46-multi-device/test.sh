#!/bin/bash
DESCRIPTION="Zig replicate/verify: Two databases watching separate folders, both connected to one remote, each end up with the other's photos"
...
invoke_command "Verify the remote" "$ZIG_CLI_COMMAND verify --db $REMOTE_DB --yes"
