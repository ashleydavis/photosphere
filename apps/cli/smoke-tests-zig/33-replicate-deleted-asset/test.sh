#!/bin/bash
DESCRIPTION="Zig replicate/verify: Test replicate database with deleted asset"
...
test_replicate_with_deleted_asset() {
    ...
    invoke_command "Verify source database after replication" "$(get_zig_cli_command) verify --db $source_dir --yes" 0 "verify_source_output"
    invoke_command "Verify replica database after replication" "$(get_zig_cli_command) verify --db $replica_dir --yes" 0 "verify_replica_output"
...
test_replicate_with_deleted_asset "${1:-33}"
