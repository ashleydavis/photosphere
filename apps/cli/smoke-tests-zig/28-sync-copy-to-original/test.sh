#!/bin/bash
DESCRIPTION="Zig replicate/verify: Test sync from copy to original"
...
test_sync_copy_to_original() {
    ...
    invoke_command "Verify original database after reverse sync" "$(get_zig_cli_command) verify --db $original_dir --yes" 0 "verify_original_output"
    invoke_command "Verify copy database after reverse sync" "$(get_zig_cli_command) verify --db $copy_dir --yes" 0 "verify_copy_output"
...
test_sync_copy_to_original "${1:-28}"
