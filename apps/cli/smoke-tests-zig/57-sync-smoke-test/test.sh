#!/bin/bash
DESCRIPTION="Zig replicate/verify: sync-smoke-test.sh"
...
source "$_SYNC_SCRIPT_DIR/../../../../scripts/lib/test-lib.sh"

# The Zig CLI, which runs the psi verify and psi replicate commands in this test (get_zig_cli_command).
source "$_SYNC_SCRIPT_DIR/../lib/interop.sh"

# sync-smoke-test.sh runs without NODE_ENV (bun run test:cli:*), so its parallel processes do not share the
# deterministic test UUID counter. smoke-tests-zig.sh exports NODE_ENV=testing, so it is removed here.
unset NODE_ENV
...
create_replicas() {
    ...
    for ((i=1; i<NUM_REPLICAS; i++)); do
        ...
        $(get_zig_cli_command) replicate --db "$TEST_DB_DIR" --dest "$replica_dir" --yes --force > /dev/null 2>&1
