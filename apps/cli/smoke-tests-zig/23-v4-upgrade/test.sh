#!/bin/bash
DESCRIPTION="Zig replicate/verify: Upgrade v4 database to v6"
...
test_v4_database_upgrade() {
    ...
    local verify_output
    invoke_command "Verify upgraded database" "$(get_zig_cli_command) verify --db $temp_v4_dir --yes" 0 "verify_output"
