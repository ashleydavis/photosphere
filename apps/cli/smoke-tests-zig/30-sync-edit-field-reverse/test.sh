#!/bin/bash
DESCRIPTION="Zig replicate/verify: Test sync after editing field in copy database with bdb-cli"
...
test_sync_edit_field_reverse() {
    ...
    invoke_command "Verify original database after sync" "$(get_zig_cli_command) verify --db $original_dir --yes" 0 "verify_original_output"
    invoke_command "Verify copy database after sync" "$(get_zig_cli_command) verify --db $copy_dir --yes" 0 "verify_copy_output"
...
test_sync_edit_field_reverse "${1:-30}"
