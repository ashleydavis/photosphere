#!/bin/bash
DESCRIPTION="Zig replicate/verify: Test replicate fails between unrelated databases"
...
test_replicate_unrelated_databases_fail() {
    ...
    invoke_command "Replicate unrelated databases (should fail)" "$(get_zig_cli_command) replicate --db $first_db_dir --dest $second_db_dir --yes" 1 "replicate_output"
...
test_replicate_unrelated_databases_fail "${1:-34}"
