#!/bin/bash
DESCRIPTION="Zig replicate/verify: write-lock-smoke-test.sh"
...
source "$_WRITE_LOCK_SCRIPT_DIR/../lib/interop.sh"

# write-lock-smoke-test.sh runs without NODE_ENV (bun run test:cli:*), so its parallel processes do not share the
