#!/bin/bash
DESCRIPTION="Zig replicate/verify: Test summary and verify reject v2 database (suggest upgrade)"
...
test_v2_database_readonly_commands() {
    ...
    local verify_output
    invoke_command "Run verify on v2 database (should fail)" "$(get_zig_cli_command) verify --db $v2_db_dir --yes" 1 "verify_output"
...
test_v2_database_readonly_commands "${1:-20}"
