#!/bin/bash
DESCRIPTION="Zig replicate/verify: psi add --cleanup deletes a source file the database holds, and keeps one it does not"
...
invoke_command "Verify the database" "$ZIG_CLI_COMMAND verify --db $TEST_DB_DIR --yes"
