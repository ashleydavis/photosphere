#!/bin/bash
DESCRIPTION="Zig replicate/verify: Replicate encrypted to plain (decrypted replica) (smoke-tests-encrypted.sh replicate-decrypted-from-encrypted)"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/../lib/encrypted-functions.sh"
source "$SCRIPT_DIR/../lib/interop.sh"
trap cleanup_and_show_summary EXIT

# Zig version of test_replicate_decrypted_from_encrypted from smoke-tests-encrypted.sh: psi verify and psi replicate run in the Zig CLI.
test_replicate_decrypted_from_encrypted() {
    ...
    invoke_command "Verify plain replica" "$zig_cli verify --db \"$plain_dir\" --yes" || {
        test_failed "$name"
        return
    }
