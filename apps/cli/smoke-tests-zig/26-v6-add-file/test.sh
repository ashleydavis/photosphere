#!/bin/bash
DESCRIPTION="Zig replicate/verify: Test adding file to v6 database"
...
test_v6_database_add_file() {
    ...
    local verify_output
    invoke_command "Verify database integrity after adding file" "$(get_zig_cli_command) verify --db $temp_v6_dir --yes" 0 "verify_output"
...
test_v6_database_add_file "${1:-26}"
