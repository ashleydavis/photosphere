#!/bin/bash
DESCRIPTION="Zig replicate/verify: Verify, repair and orphan handling on an S3 database"
...
test_s3_verify_repair() {
    ...
    local verify_output
    invoke_command "Verify the intact S3 database" "$(get_zig_cli_command) verify --db \"$s3_db\" --yes" 0 "verify_output"
    expect_output_string "$verify_output" "Database verification passed" "The intact S3 database verifies"

    local verify_full_output
    invoke_command "Fully verify the intact S3 database" "$(get_zig_cli_command) verify --db \"$s3_db\" --full --yes" 0 "verify_full_output"
    ...
    invoke_command "Verify after deleting an asset object" "$(get_zig_cli_command) verify --db \"$s3_db\" --yes" 1 "verify_deleted_output"
    ...
    invoke_command "Fully verify after overwriting an asset object" "$(get_zig_cli_command) verify --db \"$s3_db\" --full --yes" 1 "verify_modified_output"
