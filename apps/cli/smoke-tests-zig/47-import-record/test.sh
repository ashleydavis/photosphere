#!/bin/bash
DESCRIPTION="Zig replicate/verify: The import record remembers what a database took in, badges manual and automatic apart, stays on this machine and never appears inside a database"
...
invoke_command "Replicate to a third database" "$ZIG_CLI_COMMAND replicate --db $LOCAL_DB --dest $REPLICA_DB --yes"
