#!/bin/bash
DESCRIPTION="Zig replicate/verify: psi add imports, and psi sync pushes what it imported to the origin"
...
# The Zig CLI, which runs every psi verify and psi replicate in this test.
ZIG_CLI_COMMAND=$(get_zig_cli_command)
...
invoke_command "Replicate the local database to the origin" "$ZIG_CLI_COMMAND replicate --db $TEST_DB_DIR --dest $ORIGIN_DB_DIR --yes"
...
invoke_command "Verify the origin" "$ZIG_CLI_COMMAND verify --db $ORIGIN_DB_DIR --yes"
...
invoke_command "Verify the local database" "$ZIG_CLI_COMMAND verify --db $TEST_DB_DIR --yes"
