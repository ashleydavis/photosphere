#!/bin/bash
DESCRIPTION="Zig replicate/verify: Test partial replication (README and .db files only, no media)"
...
test_replicate_partial() {
    ...
    local verify_output
    invoke_command "Verify partial replica database" "$(get_zig_cli_command) verify --db $replica_dir --yes" 0 "verify_output"
