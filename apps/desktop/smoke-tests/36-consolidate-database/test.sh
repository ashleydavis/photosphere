...
# Exercises "Consolidate into remote" from Manage Databases against a remote that already holds a
...
# that: consolidating is the separate, explicit operation that makes them related. The two databases
...
print_test_header 36 "consolidate-database"
...
# The same photo goes into both, so consolidating has something it must not upload twice. Each of them
...
# --- Consolidate the local database into the unrelated remote. ---

send_command "$APP_PORT" click '{"dataId":"consolidate-database-button","nth":0}'
wait_for_log "$TMP_DIR" "Consolidate into remote dialog opened"

send_command "$APP_PORT" type "{\"dataId\":\"consolidate-remote-path-input\",\"text\":\"$REMOTE_DB\"}"
send_command "$APP_PORT" click '{"dataId":"consolidate-database-confirm"}'

wait_for_log "$TMP_DIR" "Consolidated into remote" 120
wait_for_value "$APP_PORT" "consolidate-database-success" "Photos uploaded: 1"
log_success "One photo was uploaded and the shared one was recognised"

send_command "$APP_PORT" click '{"dataId":"consolidate-database-close"}'
...
if [ "$REMOTE_AFTER" -ne 3 ]; then
    log_error "Expected the remote to hold 3 originals after consolidating, found $REMOTE_AFTER"
...
cd "$DESKTOP_DIR" || exit 1
log_success "Ordinary sync works after consolidating"

check_no_errors "$TMP_DIR"

stop_app "$APP_PORT" "$TMP_DIR"

log_success "Test 36 passed: consolidate-database"
