...
// IPC handler that lets the renderer's log interface fetch log file details from the
// main process log for bug reports. The log header is read from the log file on demand.
ipcMain.handle('get-log-details', logExceptions(async () => {
    if (!fileLogger) {
        return noLogDetails;
    }
    return await fileLogger.getLogDetails();
}, 'Error getting log details'));
...
//
// Opens a multi-file picker and returns the selected file paths, or undefined if cancelled.
//
async function showFilePicker(title: string): Promise<string[] | undefined> {
    if (mainWindow) {
        if (mainWindow.isMinimized()) {
            mainWindow.restore();
        }
