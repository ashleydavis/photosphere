...
# Create an empty database and seed it into the app sandbox, and add a database
...
# appears and tears the menu down, so wait for that render too. When this races, the share dialog
# never opens and the failure surfaces as the misleading "Failed to read pairing code from sender".
