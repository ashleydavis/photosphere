...
pub const IDatabaseFileVerifyResult = node_api.verify.IDatabaseFileVerifyResult;
pub const IVerifyResult = node_api.verify.IVerifyResult;
...
//
pub fn verifyFoundProblems(result: IVerifyResult, databaseFiles: ?IDatabaseFileVerifyResult) bool {
    return (if (databaseFiles) |files| files.invalidFiles.len else 0) > 0 or
        result.modified.len > 0 or
        result.new.len > 0 or
        result.removed.len > 0 or
        result.numFailures > 0 or
        (if (result.recordMismatches) |recordMismatches| recordMismatches.len else 0) > 0;
}
...
pub fn verifyCommand(allocator: std.mem.Allocator, io: std.Io, context: ICommandContext, options: *IVerifyCommandOptions) !void {
    ...
    // pass.
    exit(io, if (hasProblems) 1 else 0);
}
