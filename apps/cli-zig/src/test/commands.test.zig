...
test "verify reports a modified file like TypeScript" {
    ...
    const tsResult = try normalize(allocator, try runTs(allocator, environment, &.{ "verify", "--db", dbTs, "--yes", "--full" }), dbTs, "<db>");
    const zigResult = try normalize(allocator, try runZig(allocator, environment, &.{ "verify", "--db", dbZig, "--yes", "--full" }), dbZig, "<db>");
    try expectSameResult(tsResult, zigResult);
