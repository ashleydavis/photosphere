...
pub const WorkerPoolBun = struct {
    ...
    //
    pub fn addTask(self: *WorkerPoolBun, allocator: std.mem.Allocator, io: std.Io, @"type": []const u8, data: std.json.Value, source: []const u8, taskId: ?[]const u8) ![]const u8 {
        ...
        self.lock();
        defer self.unlock();
        try self.pendingTasks.append(pool_allocator, poolTask);
    ...
    fn workerMain(self: *WorkerPoolBun, workerState: *IWorkerState) void {
        ...
        while (true) {
            ...
            poolTask.context = TaskContext.init(uuidGenerator, timestampProvider, sessionId, poolTask.task.id, .{ .context = poolTask, .function = sendMessageFn });
            self.unlock();
