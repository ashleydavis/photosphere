...
pub fn getDefaultS3Config(allocator: std.mem.Allocator, io: std.Io) !?IS3Credentials {
    const accessKeyId = process_env.getEnv("AWS_ACCESS_KEY_ID") orelse "";
    const secretAccessKey = process_env.getEnv("AWS_SECRET_ACCESS_KEY") orelse "";
    if (accessKeyId.len > 0 and secretAccessKey.len > 0) {
        const region = process_env.getEnv("AWS_REGION") orelse "";
        return .{
            .region = if (region.len > 0) region else "us-east-1",
