...
//
// The options of the program itself (`.option('--version', ...)`, `.option('--debug', ...)` and
// `.option('-q, --quiet', ...)`).
//
pub const programOptions = [_]OptionSpec{
    ...
    .{ .flags = "-q, --quiet", .description = "Suppress optional output (update and news notifications). Give it before the command name." },
};

//
// A command to run: its options and `program.opts().quiet === true` (the program's --quiet flag, read by
// the preAction hook).
//
pub fn Invocation(comptime OptionsT: type) type {
    return struct {
...
pub const ParseOutcome = union(enum) {
    ...
    replicate: Invocation(IReplicateCommandOptions),

    // Run the verify command with these options.
    verify: Invocation(IVerifyCommandOptions),
};
...
pub fn parseCommandLine(allocator: std.mem.Allocator, userArgs: []const []const u8) !ParseOutcome {
    ...
    const quiet = flagValue(&programValues, "quiet") orelse false;
    if (std.mem.eql(u8, spec.name, replicateSpec.name)) {
        return .{ .replicate = .{ .quiet = quiet, .options = .{
    ...
    return .{ .verify = .{ .quiet = quiet, .options = .{
        .base = baseOptions(&values),
        .full = flagValue(&values, "full"),
        .path = textValue(&values, "path"),
    } } };
...
fn run(allocator: std.mem.Allocator, io: std.Io, userArgs: []const []const u8) !u8 {
    ...
    switch (outcome) {
        ...
        .replicate => |parsed| {
            ...
            // The preAction hook (skipped only for the news and bug commands). --quiet suppresses the
            // notifications for any command, which is what a caller reading the output by machine wants.
            try print_notifications.printNotifications(allocator, io, parsed.quiet);
