...
## `test:and` makes sure an emulator is ready, and shares the emulators
...
- **Brings the emulator up itself when none is on the LAN bridge**, by running the same `emulator.sh up` that `bun run emu:and:up` runs, and then waits for it. That asks for sudo when the bridge itself is down, and for nothing when it is already up. It starts an emulator and nothing else: a running pool is left exactly as it is, nothing is restarted, wiped or reconfigured, and an emulator that is up but off the bridge is reported rather than killed, because `-wifi-tap` is fixed at launch and the run then stops and tells you to `bun run emu:and:restart`. This is still the human's command when you run it yourself; `test:and` running it for you does not make it yours. The one exception is `PHOTOSPHERE_NO_LAN_BRIDGE=1`, which declares that this run cannot have a bridge at all: it then requires only a started emulator, starts nothing (the release workflow's emulator is booted by an action and there is no sudo there for a bridge), and `26-receive-database` / `27-receive-secret` log a `SKIP` line instead of failing. Do not set it locally: without a bridge you lose exactly the two tests that cover host-to-device LAN sharing.
