...
`) + source + `return __p
  ...
  // ../utils/src/lib/reverse-geocode.ts
  var import_axios = __toESM(require_axios2(), 1);
  // ../utils/src/lib/wrapped-error.ts
...
`);
  ...
  var v4_default = v4;
  // ../utils/src/lib/random-uuid-generator.ts
  class RandomUuidGenerator {
    generate() {
      return v4_default();
    }
  }
  // ../utils/src/lib/image.ts
  var import_lodash = __toESM(require_lodash(), 1);
  // ../utils/src/lib/timestamp-provider.ts
  ...
  class TaskQueue {
    ...
    awaitAllResolvers = [];
    awaitTaskResolvers = new Map;
    unsubscribeFunctions = [];
    constructor(uuidGenerator, source) {
      this.uuidGenerator = uuidGenerator;
      this.source = source;
      this.backend = getQueueBackend();
      this.unsubscribeFunctions.push(this.backend.onTaskAdded(this.source, (taskId) => {
        this.trackedTaskIds.add(taskId);
        this.numTasksInFlight++;
      }));
      this.unsubscribeFunctions.push(this.backend.onTaskComplete((result) => {
        if (!this.trackedTaskIds.has(result.taskId)) {
          return;
        }
        this.notifyCompletionCallbacks(result);
      }));
      this.unsubscribeFunctions.push(this.backend.onAnyTaskMessage((message) => {
        if (!this.trackedTaskIds.has(message.taskId)) {
          return;
        }
        this.notifyMessageCallbacks(message.taskId, message.message);
      }));
      this.unsubscribeFunctions.push(this.backend.onTasksCancelled(this.source, () => {
    ...
    onTaskMessage(messageType, callback) {
    ...
    async notifyCompletionCallbacks(result) {
    ...
    async notifyMessageCallbacks(taskId, message) {
  ...
  // ../task-queue/src/lib/worker-queue-backend.ts
  class WorkerQueueBackend {
    postMessage;
    uuidGenerator;
    completionCallbacks = [];
    messageCallbacks = [];
    anyMessageCallbacks = [];
    taskAddedCallbacks = new Map;
    tasksCancelledCallbacks = new Map;
    constructor(postMessage, uuidGenerator) {
      this.postMessage = postMessage;
      this.uuidGenerator = uuidGenerator;
    }
    addTask(type, data, source, taskId) {
      const id = taskId ?? this.uuidGenerator.generate();
    ...
    onTasksCancelled(source, callback) {
    ...
    shutdown() {}
    async notifyTaskCompleted(result) {
      for (const callback of this.completionCallbacks) {
        await callback(result);
      }
    }
    async notifyTaskMessage(taskId, message) {
      const messageType = message && typeof message === "object" && "type" in message ? message.type : undefined;
      for (const { messageType: filterType, callback } of this.messageCallbacks) {
        if (messageType !== filterType) {
          continue;
  ...
  function getRegisteredHandlerTypes() {
    return Array.from(handlers.keys());
  }
  async function executeTaskHandler(taskType, data, context) {
  ...
  function resolveHostFunction(rawHost, name, platform) {
    const candidate = rawHost[name];
    if (typeof candidate === "function") {
      return candidate.bind(rawHost);
    }
    return function notImplementedHostFunction() {
      throw new Error(notImplementedMessage(name, platform));
    };
  }
  function buildHost(rawHost) {
    const platform = rawHost.platform;
    const effectiveHost = { ...rawHost };
    for (const name of EXPECTED_HOST_FUNCTIONS) {
      effectiveHost[name] = resolveHostFunction(rawHost, name, platform);
    }
  ...
  function createTaskContext(taskId, host) {
    const context = {
  ...
  async function runTask(taskId, type, dataJson) {
