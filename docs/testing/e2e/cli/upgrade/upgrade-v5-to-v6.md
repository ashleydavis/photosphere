# CLI Manual Test: Upgrade v5 Database to v6

Test that `upgrade` brings a v5 database to the current schema version (6).
...
### 2. Copy the v5 fixture to a working directory

```bash
cp -r ../../test/dbs/v5 /tmp/psi-test/upgrade
```
