...
7. [x] **Make `apps/android-frontend/scripts/emulator.sh` bring up N emulator instances.** Status: completed, committed as `54754cfd`. Originally abandoned as unmeasurable, then reopened when the pool was authorised. **No performance of its own, and none claimed: it is the tooling that makes items 8-16 measurable at all.** `PHOTOSPHERE_EMULATOR_COUNT` drives the instance count, with `bun run emu:and:up` for one writable emulator and `bun run emu:and:pool` for five read-only ones. Verified read-only against the running emulator: `status` reports it correctly and `pool` refuses cleanly, before asking for sudo, rather than touching an emulator it does not own. Gives each emulator its own tap interface on the shared bridge and lets several instances boot from one AVD, which is what makes running more than one emulator possible at all. **Impact: no speedup on its own. `-wifi-tap` binds one emulator to one tap and an AVD cannot boot twice without `-read-only`, so this is a hard prerequisite for Phase 3.**
...
8. [x] **Make `emulator.sh` `cmd_status` report per-device.** Status: completed, committed as `82801181`. Per-device readiness reporting, needed so the pool knows which emulators it may use. No performance of its own. Depends on step 7, which is abandoned: with one attached emulator a per-device readiness report is the same one line `status` already prints. Reports readiness for every attached emulator rather than assuming one, so the human can see which instances are actually on the bridge. **Impact: no speedup. The pool cannot pick devices without it.**
...
9. [x] **Add device enumeration to `apps/smoke-tests/lib/android.sh`.** Status: completed, committed as `058474d5`. `android_ready_devices` / `android_device_slots` / `android_export_device`, plus `PHOTOSPHERE_ANDROID_DEVICES` to pin a run to chosen emulators. Exporting `ANDROID_SERIAL` per worker is what lets the two tests calling `adb` directly work unchanged. Depends on step 7, which is abandoned: with one attached emulator the ready-device list has exactly one entry, which is what the existing code already assumes. Gives the runner the list of usable emulator serials and a way to bind a worker to one of them. **Impact: no speedup. Exporting `ANDROID_SERIAL` per worker is also what lets the two tests calling `adb` directly keep working unmodified.**
...
10. [x] **Add matching device-slot hooks to `apps/smoke-tests/lib/ios.sh`.** Status: completed, committed as `058474d5`. `ios_device_slots` / `ios_export_device`, a single slot, so the shared pool does not regress `bun run test:ios`. Depends on step 7, which is abandoned. iOS was always single-slot, so this step only ever existed to keep the shared pool from regressing a pool that is not being built. Gives iOS the same interface with exactly one slot, so the shared runner works on both platforms without special-casing. **Impact: no speedup. It stops the shared runner from regressing `bun run test:ios`.**
...
11. [x] **Install onto every device.** Status: completed, committed as `058474d5`. Install and cleanup now run per device slot. `android_prepare` also stopped waiting on a single implicit device, which failed outright with `more than one device/emulator` once a pool was attached. Depends on step 7, which is abandoned: with one device there is nothing to install onto but the one device, which `android_install` already does. Makes install and cleanup act on one named device so the prologue can put the APK on each emulator the pool will use. **Impact: slightly negative in isolation, since the prologue now installs N times instead of once. Step 12 recovers that many times over.**
...
12. [x] **Create `apps/smoke-tests/lib/runner.sh`.** Status: completed, committed as `058474d5`. **The main win, and it delivered: 276s to 113s on six emulators (113s, 111s), 38/38 passing both runs. 163s faster, 59% off the single-emulator time, and 3.3x against the original 375s.** The pool itself, and the reason the rest of Phases 2 and 3 exist. **Cannot be shown to deliver anything here: with one device slot `run_pool` runs exactly one worker, which is the sequential loop it replaces.** The estimated 250s assumed three emulators. Adds a shared work queue that hands each test to exactly one worker, so N emulators chew through the suite at once instead of one emulator doing all 38 in series. **Impact: the main win, roughly 250s off the loop at three emulators. Everything in Phase 2 exists to make this step possible.**
...
13. [x] **Create `apps/smoke-tests/runner.test.sh`.** Status: completed, committed as `058474d5`. 30 assertions, no device needed, including that a queue entry is never handed to two workers, that `.exclusive` tests never overlap, and that ordinary ones do. Depends on step 12, which is abandoned. (The tests were written and did pass, 30 assertions including proof that `.exclusive` tests never overlap and ordinary ones do, but they cover code that is not being kept.) Covers the queue and the exclusive lock directly, proving a test is never handed to two workers and that exclusivity serialises only what it should. **Impact: no speedup. These are the parts most likely to produce intermittent, hard-to-reproduce failures if they are subtly wrong.**
...
14. [x] **Rewrite `main` in `apps/smoke-tests/run.sh` to use the pool.** Status: completed, committed as `058474d5`. This is what turned step 12 from library code into the measured 163s. Depends on step 12, which is abandoned: rewiring `run.sh` around a pool that runs one worker changes nothing but adds risk to a suite that currently passes 38/38. Swaps the sequential loop over tests for the worker pool across device slots, which is what turns step 12 from library code into an actual speedup. **Impact: no speedup of its own; this is the step that activates step 12.**
...
15. [x] **Redirect per-test output to a log file when more than one worker is running.** Status: completed, committed as `058474d5`. With six workers the interleaved output was unreadable; each test now writes `tmp/test-run.log` and only its status line is printed, and failures are listed with their log paths. A single device still streams to the terminal. Depends on step 12, which is abandoned: with one worker the output does not interleave, so there is nothing to fix and redirecting it would lose the live output the single-emulator run currently shows. Keeps the console to one `RUN`/`PASS`/`FAIL` line per test, since concurrent workers otherwise interleave their output into something unreadable. **Impact: no speedup. Without it a parallel failure is very hard to diagnose.**
...
16. [x] **Add the exclusivity markers.** Status: completed, committed as `058474d5`. **Load-bearing, not cosmetic: all six emulators share one broadcast segment, so without `.exclusive` the LAN tests corrupt each other.** `.slow` on test 37 also stops its 69s being the last thing running. Depends on step 12, which is abandoned: the markers only mean something to the pool's scheduler. With one worker nothing runs concurrently, so no LAN test can collide and ordering the slow test first saves nothing. Marks the six LAN-sharing tests so only one runs at a time across the whole pool, and marks the longest test so it starts first rather than last. **Impact: prevents guaranteed false failures once more than one emulator is in play, and removes up to 74s of tail latency.**

    Create an empty `.exclusive` file in each of `apps/smoke-tests/tests/7-share-secret/`, `8-share-database/`, `26-receive-database/`, `27-receive-secret/`, and `37-lan-share-timeout/`. All five put a LAN-share sender or receiver on the shared `192.168.55.0/24` segment, where discovery is a UDP broadcast to `255.255.255.255:54321`; two emulators on the one bridge hear each other, and `37-lan-share-timeout` specifically asserts that *no* receiver is found, so a concurrent `26` would break it. Create an empty `.slow` file in `apps/smoke-tests/tests/37-lan-share-timeout/`. Add a short `README.md` in `apps/smoke-tests/tests/` documenting both markers.

17. [x] **Update the docs.** Status: completed. `docs/testing/README.md`, `apps/smoke-tests/tests/README.md` and `apps/android-frontend/CLAUDE.md` now cover the pool commands, the markers and `PHOTOSPHERE_ANDROID_DEVICES`. Depends on step 7, which is abandoned: documenting how to run several emulators is pointless when the code to use them is not being kept. Records how to bring up more than one emulator and what the new markers and environment variables mean. **Impact: no speedup. Without it nobody knows to start a second emulator, so the parallelism goes unused.**
...
| Configuration | Time | 38/38 passing |
| --- | --- | --- |
| Original, one emulator | 375s (376s, 374s) | yes |
| One emulator, after the poll-interval fix (items 1-2) | 276s | yes |
| Six emulators, worker pool (items 7-17) | **113s** (113s, 111s) | yes |

**3.3x faster overall.** `37-lan-share-timeout` at 69s is now the floor: it deliberately waits out a real 60 second LAN-share window, and it is `.exclusive`, so nothing overlaps it. Shortening it is the next lever, and needs the worker-config change described in the Notes.

Phases 2 and 3 were abandoned once, when only one emulator was available and the pool could not be shown to beat the sequential loop it replaces. They were reopened and completed after the pool was authorised. The record of that is kept deliberately: the difference between "measurement refuted this" (items 3-6) and "this could not be measured yet" (items 7-17 at the time) is worth being able to tell apart later.

### Notes on the earlier abandonment

The worker pool was the plan's biggest estimated win (-250s at three emulators), and it was abandoned for one reason: **there is no way to demonstrate it here.** `adb devices` reports a single emulator, and `apps/android-frontend/CLAUDE.md` makes starting, stopping or restarting an emulator the human's job, not the harness's. With one device the pool runs one worker, which is precisely the sequential loop it would replace, so its measured improvement is zero and there is nothing worth committing.

This is a blocked step, not a disproved one. Unlike items 3 to 6, where measurement showed the idea itself was worth far less than estimated, the pool's value is untested rather than refuted. To revisit it, a human needs to bring up a second and third emulator on the LAN bridge; then Phases 2 and 3 can be implemented and measured against the 276s single-emulator figure. The work already proved out in this session, and discarded with the rest, was `lib/runner.sh` plus a 30-assertion `runner.test.sh` covering the queue and the exclusivity lock, so the scheduling core is known to be writable and testable without a device.

The remaining single-emulator lead, now that the poll interval is fixed, is the per-test app restart: every one of the 38 tests starts a fresh control bridge and cold-launches the app. `stop_app` alone spends a fixed 1 second per test sleeping between `SIGTERM` and `SIGKILL` of the bridge, which is roughly 38s of the remaining 276s. That is measurable on one emulator and is the obvious next thing to look at.
