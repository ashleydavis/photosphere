...
4. [ ] **Add an explicit build-skip escape hatch for iteration.** Status: not started. Lets a run that is only changing the test scripts skip the web build, Capacitor sync, and Gradle assemble entirely. **Impact: 60-120s per run, but only when the variable is explicitly set. Zero on a normal run.**
