...
7. [x] **Make `apps/android-frontend/scripts/emulator.sh` bring up N emulator instances.** Status: abandoned, not implemented. **Unmeasurable and therefore uncommittable: `adb devices` shows one emulator, and starting a second is explicitly the human's job (`apps/android-frontend/CLAUDE.md`, "DO NOT touch the emulator"), which forbids the harness or me from launching one.** Every following step in Phases 2 and 3 exists only to feed the worker pool, and the pool's entire benefit is having more than one device to spread work over. See the note at the end of Phase 3. Gives each emulator its own tap interface on the shared bridge and lets several instances boot from one AVD, which is what makes running more than one emulator possible at all. **Impact: no speedup on its own. `-wifi-tap` binds one emulator to one tap and an AVD cannot boot twice without `-read-only`, so this is a hard prerequisite for Phase 3.**
