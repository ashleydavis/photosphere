...
- Writing through a file description this process opened itself, by opening `/dev/stdout`, delivers all 220,890 bytes. Its flags are its own rather than the ones the Workers brought.
...
2. **Write the CLI's output through a file description of its own.** `apps/cli/src/lib/console-output.ts` exports `writeOutputLine` and `writeErrorLine`, which open `/dev/stdout` and `/dev/stderr` once and write through them, falling back to the console where those paths do not exist (Windows, which has never shown the loss) or cannot be opened. `apps/cli/src/lib/log.ts` and `apps/cli/src/lib/worker-log-bun.ts` call them for every line they print, which is every line the CLI prints: no command writes to the console directly.
