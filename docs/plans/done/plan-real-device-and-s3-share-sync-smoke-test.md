...
- Step 8 executed: `bun run test:and:unit` needs no change, confirmed by reading the scripts. Nothing in the repository runs `connectedAndroidTest`.
- The `synced` flag is weaker than step 27 claims. `synced: true` only means `syncDatabases` did not take its content-hash early-out; `syncDatabase` has a second early return when the two BSON merkle trees hash equal, and that path still reports `synced: true`. So "Sync completed: changes synced" says a sync ran, not that any record moved. Test 45 therefore does not rely on it alone: it reads the origin back with the CLI, and reports the origin's root hash on either side of the sync so a sync that wrote nothing is distinguishable from one that wrote the wrong thing.
- Writing this test surfaced three defects that were fixed rather than worked around. The mobile asset server's socket shim declared neither `readable` nor `writable`, so on-finished judged every request finished before it was read and every JSON-body route returned 500: metadata editing on device had never worked. `AssetInfo` seeded its description once at mount and never looked at the asset again, so a panel opened before the asset had loaded showed a blank description permanently and the next edit would have written that blank over the real value. And `persistDatabaseOps` logged nothing, so a failed metadata write left no trace anywhere.
- Two of test 45's own failures were ambiguous log waits, not product faults. Both databases in the test hold exactly one asset, so "Gallery loaded: 1 assets" and "Gallery items rendered" are emitted identically by the replica and by the origin, and `wait_for_log`'s cursor matched a render belonging to the database being navigated away from. "Database opened" is no better as an anchor because it races "Gallery items rendered". The line that works is the load-start, which names the database and precedes both.
