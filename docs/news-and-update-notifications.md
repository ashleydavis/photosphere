...
Update notifications are persisted via `news.last_shown_update_version` in `config.yaml`. The CLI silences repeat notifications for the same version, and the desktop main process suppresses sending the `update-available` IPC when the version has already been recorded.
...
- Shown IDs are persisted in the `news` section of `~/.config/photosphere/config.yaml`, under `shown_news_ids`, and are shared between the desktop app and the CLI on the same machine. The same section also stores `last_shown_update_version` for update notifications. Path is overridable via `PHOTOSPHERE_CONFIG_DIR`. See the [configuration file](https://github.com/ashleydavis/photosphere/wiki/Configuration-File) in the wiki. Not to be confused with the published `news.yaml` feed above, which is fetched over the network.
...
2. `getShownNewsIds()` / `addShownNewsIds()` (plus `getLastShownUpdateVersion()` / `setLastShownUpdateVersion()` for the update side) in [`packages/node-api/src/lib/news-state.ts`](../packages/node-api/src/lib/news-state.ts) track which items and which update version have been shown. State lives in the `news` section of `config.yaml`, beside every other setting.
