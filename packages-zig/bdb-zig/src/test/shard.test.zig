...
test "BsonShard flush throws when dirty" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var storage = MemoryStorage.init(allocator);
    var shard = newTestShard(allocator, "s1", &storage);
    try shard.setRecord(io, recordId1, try makeRecord(allocator, recordId1, "a"));
    try std.testing.expectError(error.Thrown, shard.flush());
    try std.testing.expectEqualStrings("Shard s1 is dirty, can't flush the cache.", errors.lastErrorMessage());
}

test "BsonShard flush clears in-memory records after commit" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var storage = MemoryStorage.init(allocator);
...
test "BsonShard flush followed by setRecord reloads from storage first" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    var storage = MemoryStorage.init(allocator);
