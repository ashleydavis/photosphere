...
fn WriteOperation(comptime StorageT: type) type {
    return struct {
        ...
        pub const source = "() => storage.write(filePath, void 0, finalBuffer)";
...
fn ReadOperation(comptime StorageT: type) type {
    return struct {
        ...
        // The Bun toString() of the TypeScript arrow function (used in the retry timeout message).
        pub const source = "() => storage.read(filePath)";
...
pub fn save(
    ...
    try retry(io, &operation, 3, 1_000, 2, SERIALIZED_FILE_TIMEOUT_MS, try std.fmt.allocPrint(allocator, "Failed to write {s}", .{filePath}));
}
...
pub fn load(
    ...
    const buffer = try retry(io, &readOperation, 3, 1_000, 2, SERIALIZED_FILE_TIMEOUT_MS, try std.fmt.allocPrint(allocator, "Failed to read {s}", .{filePath})) orelse {
        return null;
    };
