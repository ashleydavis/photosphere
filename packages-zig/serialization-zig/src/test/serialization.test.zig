...
// ---------------------------------------------------------------------------------------------------------------
// retries of the storage read and write
// ---------------------------------------------------------------------------------------------------------------

//
// A storage whose every read and write fails.
//
const FailingStorage = struct {
    //
    // Fails to read.
    //
    pub fn read(self: *FailingStorage, allocator: std.mem.Allocator, ioArgument: std.Io, filePath: []const u8) !?[]u8 {
        _ = self;
        _ = allocator;
        _ = ioArgument;
        return errors.throwError("cannot read {s}", .{filePath});
    }

    //
    // Fails to write.
    //
    pub fn write(self: *FailingStorage, allocator: std.mem.Allocator, ioArgument: std.Io, filePath: []const u8, contentType: ?[]const u8, data: []const u8) !void {
        _ = self;
        _ = allocator;
        _ = ioArgument;
        _ = contentType;
        _ = data;
        return errors.throwError("cannot write {s}", .{filePath});
    }
};
...
test "save names the file it could not write once every retry has failed" {
    ...
    defer arena.deinit();
    const allocator = arena.allocator();
    var storage: FailingStorage = .{};

    // (Zig: the retry warnings are captured, because the test runner fails a test that writes to stderr.)
