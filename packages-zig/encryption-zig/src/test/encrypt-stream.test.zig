...
test "fails the stream rather than writing a file nothing can decrypt" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
...
test "fails an empty stream too, because flushing one still writes a file" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const smallPublicKey = try crypto.createPublicKey(allocator, try helpers.readFixture(allocator, "ts-small-public.pem"));
...
test "fails when the new-format header is there and no key in the map matches it" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const keys = try loadTestKeys(allocator);
    ...
    const emptyMap: IPrivateKeyMap = .empty;
    try std.testing.expectError(error.ReadFailed, decryptThroughStream(allocator, &emptyMap, encrypted));
    const expected = try std.fmt.allocPrint(allocator, "Could not decrypt data that says it is encrypted: no private key for key hash {s}", .{std.fmt.bytesToHex(encrypted[12..44].*, .lower)});
    try std.testing.expectEqualStrings(expected, utils.errors.lastErrorMessage());
}

test "fails when the new-format header is there and the map holds the wrong key" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const keys = try loadTestKeys(allocator);
    ...
    try std.testing.expectError(error.ReadFailed, decryptThroughStream(allocator, &wrongMap, encrypted));
...
test "decrypts correctly for every chunk size that splits the header" {
    ...
    for (chunkSizes) |chunkSize| {
        ...
        try std.testing.expectError(error.Thrown, decryptInChunks(allocator, &emptyMap, encrypted, chunkSize));
