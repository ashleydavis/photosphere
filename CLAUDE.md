...
- Never invoke shell scripts directly (e.g. `./apps/smoke-tests/run.sh`). Use the `bun run` equivalent from `package.json` (e.g. `bun run test:electron`, `bun run test:and`, `bun run test:cli`).
