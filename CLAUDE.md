...
- When a shell script needs something shell cannot do, do it in shell anyway wherever that is possible: `stat` for a modification time, `cmp` for an exact file comparison, `uuidgen` for a UUID. Only when there is genuinely no shell answer (binding a socket to find a free port, encoding a string as JSON without getting the escaping wrong) does it become a TypeScript helper: a real `.ts` file under `scripts/`, with a comment block saying what it is for and a documented argument list, invoked as `bun scripts/thing.ts <args>`. See `scripts/seed-s3-bucket.ts` for the shape. That file gets type-checked, read and tested like the rest of the codebase. A string inside a shell script gets none of that.
...
- `bun run test:everything` (alias `bun run tev`) - Run the genuine full set for this platform, all at once (compile, unit, CLI, Electron, both mobile suites, and the what-changed smoke tests). Use this when asked to run "all tests". Runs are gated on changed paths: a script is only run when the paths it watches (see `what-changed.json`) differ from what they were the last time that script passed, so a docs-only change runs nothing. Pass `-- --force` to run everything regardless, `-- --plan` to print the decision without running anything. Pass script names to consider only those, still in parallel. This is what the git hooks run; see `docs/git-hooks.md`.
- `bun run test:everything:all` - The ungated parallel runner, bypassing what-changed entirely.
