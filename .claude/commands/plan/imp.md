...
0. **Choose the plan** — if a specific plan is obvious from the conversation context, use that. Otherwise list the 5 most recent files in `docs/plans/` (by modification time) and present them as a numbered menu for the user to choose from. Wait for the user's selection before continuing.
...
2. **Read the plan** — read the chosen plan file from `docs/plans/`.
...
7. **Report** — summarise what was implemented and flag anything that was skipped or deferred.
