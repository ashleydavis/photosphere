...
test "should retry when operation times out" {
    ...
    try std.testing.expectEqualStrings("Operation timed out after 50ms", errors.lastErrorMessage());
    try std.testing.expectEqual(@as(u32, 3), operation.calls);
}
...
test "should wrap error with errorContext when provided" {
    ...
    try std.testing.expectEqualStrings("context message", errors.lastErrorMessage());
...
//
// A log with verbose logging enabled that keeps the verbose messages it receives.
//
const VerboseLog = struct {
    ...
    //
    // The ILog functions of this log: only verbose and verboseEnabled do anything.
    //
...
test "should log the error of a failed attempt when verbose logging is enabled" {
    const io = std.testing.io;
    var verbose_log: VerboseLog = .{ .messages = std.Io.Writer.Allocating.init(std.testing.allocator) };
    ...
    try std.testing.expectEqualStrings(
        "Operation failed with error, will retry.\nError: {\n  \"name\": \"Error\",\n  \"message\": \"First \\\"failure\\\"\"\n}\n",
