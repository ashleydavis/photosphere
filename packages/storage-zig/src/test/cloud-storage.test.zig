...
test "CloudStorage lists files and directories with pagination like S3" {
    ...
    try std.testing.expectError(error.Thrown, cloudStorage.listFiles(allocator, io, "/db", 1000, null));
    try expectNames(&.{"root.txt"}, (try cloudStorage.listFiles(allocator, io, "bucket//", 1000, null)).names);
    try expectNames(&.{"db"}, (try cloudStorage.listDirs(allocator, io, "bucket//", 1000, null)).names);
...
test "CloudStorage wraps S3 errors like TypeScript" {
    ...
    try std.testing.expectEqualStrings("Failed to read bucket/file: The request signature we calculated does not match the signature you provided.: The request signature we calculated does not match the signature you provided.", utils.errors.lastErrorMessage());
    ...
    try std.testing.expectEqualStrings("Failed to list files in bucket/dir: The request signature we calculated does not match the signature you provided.: The request signature we calculated does not match the signature you provided.", utils.errors.lastErrorMessage());
    try std.testing.expectError(error.Thrown, cloudStorage.listDirs(allocator, io, "bucket/dir", 10, null));
    try std.testing.expectEqualStrings("Failed to list directories in bucket/dir: The request signature we calculated does not match the signature you provided.: The request signature we calculated does not match the signature you provided.", utils.errors.lastErrorMessage());
    try std.testing.expectError(error.Thrown, cloudStorage.fileExists(allocator, io, "bucket/file"));
    try std.testing.expectEqualStrings("Failed to check if file exists: Forbidden: Forbidden", utils.errors.lastErrorMessage());
    try std.testing.expectError(error.Thrown, cloudStorage.dirExists(allocator, io, "bucket/dir"));
    try std.testing.expectEqualStrings("Failed to check if directory exists: The request signature we calculated does not match the signature you provided.: The request signature we calculated does not match the signature you provided.", utils.errors.lastErrorMessage());
    try std.testing.expectError(error.Thrown, cloudStorage.info(allocator, io, "bucket/file"));
    try std.testing.expectEqualStrings("Failed to get info for bucket/file: Forbidden: Forbidden", utils.errors.lastErrorMessage());
    try std.testing.expectError(error.Thrown, cloudStorage.write(allocator, io, "bucket/file", null, "x"));
    try std.testing.expectEqualStrings("Failed to write to bucket/file: The request signature we calculated does not match the signature you provided.: The request signature we calculated does not match the signature you provided.", utils.errors.lastErrorMessage());
    ...
    try std.testing.expectEqualStrings("Failed to write stream to bucket/file: The request signature we calculated does not match the signature you provided.: The request signature we calculated does not match the signature you provided.", utils.errors.lastErrorMessage());
...
test "CloudStorage reports a connection failure as the wrapped Zig error" {
    ...
    defer cloudStorage.s3.deinit();
    try std.testing.expectError(error.Thrown, cloudStorage.read(allocator, io, "bucket/file"));
    try std.testing.expectEqualStrings("Failed to read bucket/file: ConnectionRefused: ConnectionRefused", utils.errors.lastErrorMessage());
}
