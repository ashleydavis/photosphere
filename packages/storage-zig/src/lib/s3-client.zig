...
pub const S3Request = struct {
    ...
    // The body (null for none, like the SDK's undefined request body: a command with no payload).
    body: ?[]const u8,
};
...
pub const S3Client = struct {
    ...
    pub fn execute(self: *S3Client, allocator: std.mem.Allocator, io: std.Io, request: S3Request) !S3Response {
        ...
        // The photosphereUnsignedPayload middleware of CloudStorage.buildClient: a request with a body is sent
        // with "UNSIGNED-PAYLOAD" as its payload hash, and the signer then uses that instead of hashing the body.
        const bodyHash = sigv4.sha256Hex(body);
        const payloadHash: []const u8 = if (request.body != null) "UNSIGNED-PAYLOAD" else &bodyHash;
    ...
    pub fn putObject(self: *S3Client, allocator: std.mem.Allocator, io: std.Io, bucket: []const u8, key: []const u8, body: []const u8, contentType: ?[]const u8, ifNoneMatch: ?[]const u8) !void {
        ...
        if (ifNoneMatch) |ifNoneMatchValue| {
            try headers.append(allocator, .{ .name = "if-none-match", .value = ifNoneMatchValue });
        }
...
pub const Upload = struct {
    ...
    pub fn done(self: *Upload, allocator: std.mem.Allocator, io: std.Io) !void {
        ...
        if (firstPart.len < self.options.partSize) {
            try client.putObject(allocator, io, params.Bucket, params.Key, firstPart, params.ContentType, null);
            return;
        }
