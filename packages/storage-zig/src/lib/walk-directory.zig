...
//
// The retried list operation (TypeScript: `() => storage.listFiles(dirPath, 1000, next)` or the listDirs equivalent).
//
const ListOperation = struct {
    // The storage to list.
    storage: IStorage,
    ...
    // True to list directories, false to list files.
    listDirectories: bool,

    //
    // Lists one batch.
    //
    pub fn run(self: *ListOperation, io: std.Io) !IListResult {
        ...
        while (self.stack.items.len > 0) {
            ...
            if (!frame.fetched) {
                var operation: ListOperation = .{
                    ...
                    .next = frame.next,
                    .listDirectories = frame.walkingDirs,
                };
                const batch = try retry(self.io, &operation, 3, 1_000, 2, 30_000, null);
