...
describe('Size calculation with file updates', () => {
  ...
  test('updating file size updates node size', () => {
    ...
    // Verify initial size
    expect(tree.root?.size).toBe(initialSize);
    ...
    expect(tree.root?.size).toBe(updatedSize);
  ...
  test('updating file propagates size changes up the tree', () => {
    ...
    expect(tree.root?.size).toBe(initialTotalSize + sizeDifference);
  ...
  test('updating file to smaller size properly reduces tree size', () => {
    ...
    expect(tree.root?.size).toBe(6000);
    ...
    expect(tree.root?.size).toBe(4500);
  ...
  test('size is correctly maintained in a complex tree with multiple updates', () => {
    ...
    expect(tree.root?.size).toBe(totalInitialSize);
    ...
    expect(tree.root?.size).toBe(expectedNewTotal);
