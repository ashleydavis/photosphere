...
test "loadDatabaseConfig throws a WrappedError for invalid JSON" {
    ...
    try std.testing.expectEqualStrings("Failed to parse database config at .db/config.json", errors.lastErrorMessage());
...
test "saveDatabaseConfig then loadDatabaseConfig round-trips" {
    ...
    const io = std.testing.io;
    var memory_storage = MemoryStorage.init(allocator);
    try database_config.saveDatabaseConfig(allocator, io, memory_storage.storage(), IDatabaseConfig{ .origin = "fs:/a", .lastSyncedAt = "t" });
    ...
    try std.testing.expectEqualStrings("t", config.get("lastSyncedAt").?.string);
    try std.testing.expect(config.get("lastReplicatedAt") == null);
}
...
test "updateDatabaseConfig overwrites existing values in place" {
    ...
    const io = std.testing.io;
    var memory_storage = MemoryStorage.init(allocator);
    try database_config.saveDatabaseConfig(allocator, io, memory_storage.storage(), IDatabaseConfig{ .origin = "first", .lastModifiedAt = "m" });
