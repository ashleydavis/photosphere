...
// in .db/config.json, and the random IVs of encrypted files (compared after decryption).
//
...
fn zigReplicate(allocator: std.mem.Allocator, io: std.Io, counterDir: []const u8, data: IReplicateDatabaseData) ![]const u8 {
    ...
    var context = TaskContext.init(uuidGenerator.uuidGenerator(), timestampProvider.timestampProvider(), "parity", "parity-task", .{ .context = null, .function = ignoreMessage });
...
//
// Replaces the lastReplicatedAt value of a config.json text (the value is the time of the replication).
//
fn maskLastReplicatedAt(allocator: std.mem.Allocator, text: []const u8) ![]const u8 {
    var parsed = try std.json.parseFromSliceLeaky(std.json.Value, allocator, text, .{});
    if (parsed.object.getPtr("lastReplicatedAt")) |value| {
        value.* = .{ .string = "<time>" };
    }
...
fn compareReplicas(allocator: std.mem.Allocator, io: std.Io, zigDir: []const u8, tsDir: []const u8, key: ?[]const u8) !void {
    ...
    for (zigFiles) |fileName| {
        ...
        if (std.mem.eql(u8, fileName, ".db/config.json")) {
            if (!std.mem.eql(u8, try maskLastReplicatedAt(allocator, zigBytes), try maskLastReplicatedAt(allocator, tsBytes))) {
                try differences.append(allocator, fileName);
            }
            continue;
        }
...
//
// Checks that a replica reads, in TypeScript, like its source: same summary (root hashes, counts, sizes),
// database id and metadata (plus isPartial for a partial replica), and that the Zig and TypeScript verify agree.
//
fn expectReplicaMatchesSource(allocator: std.mem.Allocator, io: std.Io, sourceDir: []const u8, replicaDir: []const u8, sourceKey: ?[]const u8, replicaKey: ?[]const u8, partial: bool) !void {
    ...
    if (partial) {
        try expectedMetadata.object.put(allocator, "isPartial", .{ .bool = true });
    }
...
test "full replication of v6 writes the same replica as TypeScript" {
    ...
    try std.testing.expectEqualStrings("{\"filesImported\":1,\"copiedFiles\":3,\"copiedRecords\":1,\"prunedFiles\":[]}", result);
...
test "replicating again copies nothing, then carries over an edited record and a removed asset like TypeScript" {
    ...
    try std.testing.expectEqualStrings("{\"filesImported\":1,\"copiedFiles\":0,\"copiedRecords\":0,\"prunedFiles\":[]}", unchanged);
    ...
    try std.testing.expectEqualStrings("{\"filesImported\":1,\"copiedFiles\":0,\"copiedRecords\":2,\"prunedFiles\":[]}", edited);
