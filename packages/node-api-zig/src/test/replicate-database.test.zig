const std = @import("std");
const utils = @import("utils-zig");
const node_utils = @import("node-utils-zig");
const node_api = @import("node-api-zig");
const task_queue_zig = @import("task-queue-zig");
const helpers = @import("test-helpers.zig");
const errors = utils.errors;
const replicateDatabase = node_api.replicate_database.replicateDatabase;
const ReplicateProgressCallback = node_api.replicate_database.ReplicateProgressCallback;

//
// Records the progress strings of replicateDatabase.
//
fn recordProgress(context: ?*anyopaque, progress: []const u8) void {
    const recorder: *helpers.ProgressRecorder = @ptrCast(@alignCast(context.?));
    recorder.record(progress);
}

test "replicateDatabase runs the replicate-database task and returns its result and progress" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    _ = try helpers.setupEnvironment(io);
    try node_api.task_handlers.initTaskHandlers();
    const sourceDir = try helpers.copyTestDatabase(allocator, io, "v6");
    const root = std.fs.path.dirname(sourceDir).?;
    defer helpers.removeTempDir(io, root);
    var uuidGenerator = try node_utils.test_uuid_generator.TestUuidGenerator.init(allocator);

    var recorder: helpers.ProgressRecorder = .{ .allocator = allocator };
    const onProgress: ReplicateProgressCallback = .{ .context = &recorder, .function = recordProgress };
    const result = try replicateDatabase(allocator, io, uuidGenerator.uuidGenerator(), .{
        .sourcePath = sourceDir,
        .destPath = try std.fmt.allocPrint(allocator, "{s}/dest", .{root}),
        .partial = false,
        .force = false,
    }, &onProgress);

    try std.testing.expectEqual(@as(u64, 1), result.filesImported);
    try std.testing.expectEqual(@as(u64, 3), result.copiedFiles);
    try std.testing.expectEqual(@as(u64, 1), result.copiedRecords);
    try std.testing.expectEqual(@as(usize, 0), result.prunedFiles.len);
    try std.testing.expectEqual(@as(usize, 4), recorder.messages.items.len);
    try std.testing.expectEqualStrings("Copied 1", recorder.messages.items[0]);
    try std.testing.expectEqualStrings("Copied 3 files, 1 records", recorder.messages.items[3]);
}

test "replicateDatabase throws the task error message when replication fails" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    _ = try helpers.setupEnvironment(io);
    try node_api.task_handlers.initTaskHandlers();
    const root = try helpers.makeTempDir(allocator, io, "replicate-database-missing");
    defer helpers.removeTempDir(io, root);
    var uuidGenerator = try node_utils.test_uuid_generator.TestUuidGenerator.init(allocator);

    try std.testing.expectError(error.Thrown, replicateDatabase(allocator, io, uuidGenerator.uuidGenerator(), .{
        .sourcePath = try std.fmt.allocPrint(allocator, "{s}/missing", .{root}),
        .destPath = try std.fmt.allocPrint(allocator, "{s}/dest", .{root}),
        .partial = false,
        .force = false,
    }, null));
    try std.testing.expectEqualStrings("Failed to load merkle tree", errors.lastErrorMessage());
}

test "replicateDatabase fails when the destination is an unrelated database" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    _ = try helpers.setupEnvironment(io);
    try node_api.task_handlers.initTaskHandlers();
    const sourceDir = try helpers.copyTestDatabase(allocator, io, "v6");
    defer helpers.removeTempDir(io, std.fs.path.dirname(sourceDir).?);
    const destDir = try helpers.copyTestDatabase(allocator, io, "50-assets");
    defer helpers.removeTempDir(io, std.fs.path.dirname(destDir).?);
    var uuidGenerator = try node_utils.test_uuid_generator.TestUuidGenerator.init(allocator);

    try std.testing.expectError(error.Thrown, replicateDatabase(allocator, io, uuidGenerator.uuidGenerator(), .{
        .sourcePath = sourceDir,
        .destPath = destDir,
        .partial = false,
        .force = false,
    }, null));
    try std.testing.expect(std.mem.startsWith(u8, errors.lastErrorMessage(), "You are trying to replicate to a database that has a different ID than the source database.\nSource database ID: "));
}

test "replicateDatabaseDataToJson leaves out the optional keys that are not set" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const value = try node_api.replicate_database.replicateDatabaseDataToJson(allocator, .{ .sourcePath = "/a", .destPath = "/b", .destEncryptionKey = "key", .partial = true, .force = false });
    try std.testing.expectEqualStrings("{\"sourcePath\":\"/a\",\"destPath\":\"/b\",\"destEncryptionKey\":\"key\",\"partial\":true,\"force\":false}", try std.json.Stringify.valueAlloc(allocator, value, .{}));
}

//
// What the recording backend saw (TypeScript: makeRecordingBackend's sources and cancelled lists).
//
const Recorded = struct {
    // The source of each task added.
    sources: std.ArrayList([]const u8) = .empty,

    // The data of each task added, as JSON text.
    inputs: std.ArrayList([]const u8) = .empty,

    // Each source cancelled.
    cancelled: std.ArrayList([]const u8) = .empty,

    // Allocates the recordings.
    allocator: std.mem.Allocator,
};

//
// The recordings of the current test.
//
var recorded: Recorded = undefined;

//
// The functions of the backend the recording backend forwards to.
//
var forwardedVTable: task_queue_zig.queue_backend.IQueueBackend.VTable = undefined;

//
// Records a task being added, then adds it to the real backend.
//
fn recordAddTask(ptr: *anyopaque, allocator: std.mem.Allocator, io: std.Io, taskType: []const u8, data: std.json.Value, source: []const u8, taskId: ?[]const u8, priority: ?task_queue_zig.types.TaskPriority) anyerror![]const u8 {
    try recorded.sources.append(recorded.allocator, try recorded.allocator.dupe(u8, source));
    try recorded.inputs.append(recorded.allocator, try std.json.Stringify.valueAlloc(recorded.allocator, data, .{}));
    return forwardedVTable.addTask(ptr, allocator, io, taskType, data, source, taskId, priority);
}

//
// Records a source being cancelled, then cancels it in the real backend.
//
fn recordCancelTasks(ptr: *anyopaque, source: []const u8) void {
    recorded.cancelled.append(recorded.allocator, recorded.allocator.dupe(u8, source) catch source) catch {};
    forwardedVTable.cancelTasks(ptr, source);
}

//
// Replicates test/dbs/v6 to a new destination through a backend that records what it is asked to do.
// Returns the path of the source database (a directory that has been removed again).
//
fn replicateRecorded(allocator: std.mem.Allocator, io: std.Io) ![]const u8 {
    _ = try helpers.setupEnvironment(io);
    try node_api.task_handlers.initTaskHandlers();
    const sourceDir = try helpers.copyTestDatabase(allocator, io, "v6");
    const root = std.fs.path.dirname(sourceDir).?;
    defer helpers.removeTempDir(io, root);
    var uuidGenerator = try node_utils.test_uuid_generator.TestUuidGenerator.init(allocator);

    const realBackend = try task_queue_zig.queue_backend.getQueueBackend();
    forwardedVTable = realBackend.vtable.*;
    var recordingVTable = realBackend.vtable.*;
    recordingVTable.addTask = recordAddTask;
    recordingVTable.cancelTasks = recordCancelTasks;
    recorded = .{ .allocator = allocator };
    task_queue_zig.queue_backend.setQueueBackend(.{ .ptr = realBackend.ptr, .vtable = &recordingVTable });
    defer task_queue_zig.queue_backend.setQueueBackend(realBackend);

    _ = try replicateDatabase(allocator, io, uuidGenerator.uuidGenerator(), .{
        .sourcePath = sourceDir,
        .destPath = try std.fmt.allocPrint(allocator, "{s}/backup/my-database", .{root}),
        .partial = false,
        .force = false,
    }, null);
    return sourceDir;
}

test "queues the replication under the source database's path" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    const sourceDir = try replicateRecorded(allocator, io);
    try std.testing.expectEqual(@as(usize, 1), recorded.sources.items.len);
    try std.testing.expectEqualStrings(sourceDir, recorded.sources.items[0]);
}

test "does not tag the queue with a generated id" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    const sourceDir = try replicateRecorded(allocator, io);

    // A throwaway tag would carry one of the generator's ids. The path must be used verbatim.
    for (recorded.sources.items) |source| {
        try std.testing.expectEqualStrings(sourceDir, source);
    }
}

test "cancels that same source when it shuts the queue down" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    _ = try replicateRecorded(allocator, io);

    // This is what made a second replication vanish before the pool was fixed, so it is recorded
    // here rather than left implicit.
    try std.testing.expectEqual(@as(usize, 1), recorded.cancelled.items.len);
    try std.testing.expectEqualStrings(recorded.sources.items[0], recorded.cancelled.items[0]);
}

test "tags the task with a job named after the destination that cancels by the source" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    const sourceDir = try replicateRecorded(allocator, io);
    const destPath = try std.fmt.allocPrint(allocator, "{s}/backup/my-database", .{std.fs.path.dirname(sourceDir).?});

    const inputs = try std.json.parseFromSliceLeaky(std.json.Value, allocator, recorded.inputs.items[0], .{});
    const expectedJob = try std.fmt.allocPrint(allocator, "{{\"id\":{f},\"name\":\"Replicating to my-database\",\"cancelSource\":{f}}}", .{ std.json.fmt(try std.fmt.allocPrint(allocator, "replicate:{s}", .{destPath}), .{}), std.json.fmt(sourceDir, .{}) });
    try std.testing.expectEqualStrings(expectedJob, try std.json.Stringify.valueAlloc(allocator, inputs.object.get("job").?, .{}));
}
