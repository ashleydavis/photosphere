...
fn makeContext(allocator: std.mem.Allocator, io: std.Io) !*RecordingContext {
    ...
    recording.context = TaskContext.init(recording.uuidGenerator.uuidGenerator(), recording.timestampProvider.timestampProvider(), "session-1", "replicate-task-id", .{ .context = recording, .function = RecordingContext.sendMessage }, 10);
    return recording;
}
...
test "emits a replicate-progress task message for each progress callback fired by replicate()" {
    ...
    // (The job-progress messages that follow each one are left out when the task carries no job tag.)
...
//
// The recorded job-progress messages.
//
fn jobMessages(allocator: std.mem.Allocator, recording: *RecordingContext) ![]const std.json.Value {
    var messages: std.ArrayList(std.json.Value) = .empty;
    for (recording.messages.items) |text| {
        const message = try std.json.parseFromSliceLeaky(std.json.Value, allocator, text, .{});
        if (std.mem.eql(u8, message.object.get("type").?.string, "job-progress")) {
...
test "reports its job alongside each progress string, carrying the tag it was queued with" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    ...
    }), recording.context.taskContext());

    // One job message for each of the four progress strings.
    const messages = try jobMessages(allocator, recording);
    try std.testing.expectEqual(@as(usize, 4), messages.len);
    try std.testing.expectEqualStrings(
        "{\"id\":\"replicate:/fake/dest\",\"name\":\"Replicating to dest\",\"cancelSource\":\"/fake/source\"}",
        try std.json.Stringify.valueAlloc(allocator, messages[0].object.get("job").?, .{}),
    );
    try std.testing.expectEqualStrings("Copied 1", messages[0].object.get("progressMessage").?.string);
    try std.testing.expectEqualStrings("Copied 3 files, 1 records", messages[3].object.get("progressMessage").?.string);
}

test "reports no job when the replication was queued without a tag" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    ...
    try std.testing.expectEqual(@as(usize, 0), (try jobMessages(allocator, recording)).len);
}

test "hands replicate() a way to ask whether the task has been cancelled" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    const io = std.testing.io;
    ...
    // mid-copy is noticed.
    recording.context.cancel();

    try std.testing.expectError(error.Thrown, replicateDatabaseHandler(allocator, io, try makeData(allocator, .{ .sourcePath = dirs.source, .destPath = dirs.dest, .partial = false, .force = false }), recording.context.taskContext()));
...
test "returns the IReplicationResult from replicate()" {
    ...
    try std.testing.expectEqualStrings("{\"filesImported\":1,\"copiedFiles\":3,\"copiedRecords\":1,\"prunedFiles\":[],\"missingFromSource\":[]}", try std.json.Stringify.valueAlloc(allocator, output, .{}));
}
