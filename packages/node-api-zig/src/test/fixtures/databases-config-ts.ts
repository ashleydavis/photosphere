...
//   bun run databases-config-ts.ts save <config json>   writes databases.toml with updateDatabasesConfig
//   bun run databases-config-ts.ts load                 prints loadDatabasesConfig() as JSON
//

import { updateDatabasesConfig, loadDatabasesConfig } from "node-api";
...
async function main(): Promise<void> {
    ...
    if (operation === "save") {
        await updateDatabasesConfig(() => JSON.parse(configJson));
    }
