...
describe('loadAppConfig', () => {
    ...
    test('converts snake_case document keys to camelCase TypeScript fields', async () => {
        ...
        expect(config.showFpsIndicator).toBe(true);
        expect(config.savedSearches).toEqual(['beach']);
...
describe('updateAppConfig writes snake_case sections', () => {
    ...
    test('converts camelCase fields to snake_case keys in their own sections', async () => {
        ...
        expect(document.showFpsIndicator).toBeUndefined();
        expect(document.savedSearches).toBeUndefined();
    });
});
...
describe('showFpsIndicator config round-trip', () => {
    ...
    test('loadAppConfig reads show_fps_indicator from the top level', async () => {
        mockReadYaml.mockResolvedValue({ show_fps_indicator: true });
...
//
// Saved searches are the searches the user deliberately kept, so they are a setting and belong here.
// The ones merely run are in the state file instead.
//
describe('savedSearches config round-trip', () => {
    beforeEach(() => jest.clearAllMocks());

    test('loadAppConfig reads saved_searches from the top level', async () => {
        mockReadYaml.mockResolvedValue({ saved_searches: ['beach', 'dogs'] });
    ...
    test('updateAppConfig writes savedSearches to saved_searches', async () => {
        await updateAppConfig(config => { config.savedSearches = ['beach']; });

        const document = mockWriteYaml.mock.calls[0][1];
        expect(document.saved_searches).toEqual(['beach']);
    });

    test('an entry of the wrong type is dropped and the rest are kept', () => {
        expect(yamlToAppConfig({ saved_searches: ['beach', 17, 'dogs'] as any }).savedSearches)
            .toEqual(['beach', 'dogs']);
    });
});
...
describe('updateAppConfig', () => {
    ...
    //
    // The flat view owns only some of the file. The mobile loops' pacing and the database the mobile
    // sync pushes are in the same document and appear nowhere in this view, so a desktop write that
    // rebuilt the document from the flat view alone would delete them.
    //
...
describe('yamlToAppConfig', () => {
    ...
    test('a malformed section is ignored without discarding the keys that parsed', () => {
        const config = yamlToAppConfig({
            theme: 'light',
            auto_import: 'not a section' as any,
            sync: { enabled: true },
        });
...
describe('appConfigToYaml', () => {
    ...
    test('omits absent fields rather than writing them as null, and writes no empty section', () => {
        // An empty section is not the same as no section: the file uses a `sync` section's presence
        // to say syncing has been decided, so stamping an empty one in would tell a fresh install its
        // syncing settings had already been chosen and leave syncing off with the toggles saying on.
        expect(appConfigToYaml({}, {})).toEqual({});
    });

    test('a section that has been emptied is removed rather than left behind', () => {
        expect(appConfigToYaml({}, { sync: { enabled: true } })).toEqual({});
    });

    test('round trips a config through the document and back unchanged', () => {
        const original = { theme: 'light' as const, savedSearches: ['dogs'], showFpsIndicator: false };
...
//
// The flat namespace over config.yaml. Every key this file holds is a declared field, so unlike the
// state view there is no catch-all: a key that is not a config key is routed to the state file
// instead and never reaches these.
//
describe('reading and writing one config setting by name', () => {

    test('a key is read from and written to its own field', () => {
        const config: any = {};
        setAppConfigValue(config, 'theme', 'dark');

        expect(config.theme).toBe('dark');
        expect(getAppConfigValue(config, 'theme')).toBe('dark');
    });

    test('a key nothing has been stored under reads as undefined', () => {
        expect(getAppConfigValue({}, 'theme')).toBeUndefined();
    });

    test('writing nothing removes a key rather than leaving it as it was', () => {
        const config: any = { theme: 'dark' };
        setAppConfigValue(config, 'theme', undefined);

        expect('theme' in config).toBe(false);
        expect(getAppConfigValue(config, 'theme')).toBeUndefined();
    });

    test('every setting that has a value is listed together under its own name', () => {
        const config: any = { theme: 'dark', showFpsIndicator: true };

        expect(appConfigSettings(config)).toEqual({
            theme: 'dark',
            showFpsIndicator: true,
        });
    });

    test('a setting with no value is left out rather than listed as undefined', () => {
        expect(appConfigSettings({})).toEqual({});
    });
});
