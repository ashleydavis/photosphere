...
//
// Builds a record id from a small number (valid 16 byte ids, like the TypeScript tests' ids).
//
fn recordId(allocator: std.mem.Allocator, number: u32) ![]const u8 {
    return std.fmt.allocPrint(allocator, "123e4567-e89b-12d3-a456-4266141{d:0>5}", .{number});
}
