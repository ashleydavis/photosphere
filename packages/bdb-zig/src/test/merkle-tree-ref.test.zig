...
test "flush resets loaded state so next get reloads from storage" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var state: RefState = .{ .stored = try makeTree(arena.allocator()) };
    var ref = state.makeRef(arena.allocator());
    ...
    try ref.flush();
    _ = try ref.get(io);
    try std.testing.expectEqual(@as(u32, 2), state.loadCount);
}

test "flush throws when dirty" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var state: RefState = .{ .stored = try makeTree(arena.allocator()) };
    var ref = state.makeRef(arena.allocator());
    try ref.upsert(io, makeItem("file1"));
    try std.testing.expectError(error.Thrown, ref.flush());
    try std.testing.expectEqualStrings("Cannot flush a dirty MerkleRef \u{2014} call commit() first.", utils.errors.lastErrorMessage());
}

test "flush does not throw when not dirty" {
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var state: RefState = .{ .stored = try makeTree(arena.allocator()) };
    var ref = state.makeRef(arena.allocator());
