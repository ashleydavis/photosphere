...
// V6 paths: api/tree uses collections/<name>/shards/<id>.dat and collections/<name>/collection.dat; bdb uses db.dat.
async function buildAndSaveTreeAsync(
    storage: MockStorage,
    filePath: string,
    uuid: string,
    leafNames: string[],
    typeCode: string = 'FTRE'
): Promise<void> {
    ...
    await saveTree(filePath, tree, storage, typeCode);
}
...
describe('iterateShardDifferences', () => {
    ...
    test('yields nothing when tree1 does not exist', async () => {
        ...
        await buildAndSaveTreeAsync(storage2, shardPath('coll', 's1'), VALID_UUID, ['rec1'], 'COLT');
    ...
    test('yields all record ids from tree1 when tree2 does not exist', async () => {
        const storage1 = new MockStorage();
        const storage2 = new MockStorage();
        await buildAndSaveTreeAsync(storage1, shardPath('coll', 's1'), VALID_UUID, ['rec1', 'rec2'], 'COLT');
        const results: Array<{ collectionName: string; recordId: string }> = [];
        for await (const d of iterateShardDifferences('coll', 's1', storage1, storage2)) {
            results.push(d);
        }
    ...
    test('yields differing record ids when both trees exist', async () => {
        const storage1 = new MockStorage();
        const storage2 = new MockStorage();
        await buildAndSaveTreeAsync(storage1, shardPath('coll', 's1'), VALID_UUID, ['rec1', 'rec2', 'rec3'], 'COLT');
        await buildAndSaveTreeAsync(storage2, shardPath('coll', 's1'), VALID_UUID, ['rec1'], 'COLT');
        const results: Array<{ collectionName: string; recordId: string }> = [];
        for await (const d of iterateShardDifferences('coll', 's1', storage1, storage2)) {
            results.push(d);
    ...
    test('yields nothing when both trees are identical', async () => {
        ...
        const storage2 = new MockStorage();
        await buildAndSaveTreeAsync(storage1, shardPath('coll', 's1'), VALID_UUID, ['rec1', 'rec2'], 'COLT');
        await buildAndSaveTreeAsync(storage2, shardPath('coll', 's1'), VALID_UUID, ['rec1', 'rec2'], 'COLT');
        const results: Array<{ collectionName: string; recordId: string }> = [];
        for await (const d of iterateShardDifferences('coll', 's1', storage1, storage2)) {
            results.push(d);
        }
...
describe('iterateCollectionDifferences', () => {
    ...
    test('yields nothing when tree1 collection does not exist', async () => {
        const storage1 = new MockStorage();
        const storage2 = new MockStorage();
        await buildAndSaveTreeAsync(storage2, collCollectionPath('coll'), VALID_UUID, ['s1'], 'COLT');
        const results: Array<{ collectionName: string; recordId: string }> = [];
    ...
    test('yields record ids from all shards when tree2 collection does not exist', async () => {
        const storage1 = new MockStorage();
        const storage2 = new MockStorage();
        await buildAndSaveTreeAsync(storage1, collCollectionPath('coll'), VALID_UUID, ['s1', 's2'], 'COLT');
        await buildAndSaveTreeAsync(storage1, shardPath('coll', 's1'), VALID_UUID, ['rec1'], 'COLT');
        await buildAndSaveTreeAsync(storage1, shardPath('coll', 's2'), VALID_UUID, ['rec2', 'rec3'], 'COLT');
        const results: Array<{ collectionName: string; recordId: string }> = [];
        for await (const d of iterateCollectionDifferences('coll', storage1, storage2)) {
            results.push(d);
        }
    ...
    test('yields differing record ids when both collections exist', async () => {
        ...
        await saveTree(shardPath('coll', 's1'), shardTree1, storage1, 'COLT');
        await saveTree(shardPath('coll', 's1'), shardTree2, storage2, 'COLT');
        ...
        await saveTree(collCollectionPath('coll'), collTree1, storage1, 'COLT');
...
describe('iterateDatabaseDifferences', () => {
    ...
    test('yields record ids from all collections when tree2 database does not exist', async () => {
        ...
        await buildAndSaveTreeAsync(storage1, shardPath('c2', 's1'), VALID_UUID, ['r2'], 'COLT');
        const results: Array<{ collectionName: string; recordId: string }> = [];
        for await (const d of iterateDatabaseDifferences(storage1, storage2)) {
            results.push(d);
        }
