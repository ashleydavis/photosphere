...
//
export async function isDatabasePartial(databasePath: string, s3Config?: IS3Credentials, storageOptions?: IStorageOptions): Promise<boolean> {
    const { storage } = createStorage(databasePath, s3Config, storageOptions);
