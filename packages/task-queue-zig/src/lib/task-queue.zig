...
pub const TaskQueue = struct {
    ...
    //
    pub fn addTask(self: *TaskQueue, @"type": []const u8, data: std.json.Value, taskId: ?[]const u8) ![]const u8 {
        const id = if (taskId != null and taskId.?.len > 0) taskId.? else try self.uuidGenerator.generate(self.allocator, self.io);
        _ = try self.backend.addTask(self.allocator, self.io, @"type", data, self.source, id);
        return id;
    }
