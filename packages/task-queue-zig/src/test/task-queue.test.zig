...
test "addTask: should add a task and return a UUID" {
    ...
    const taskId = try fixture.queue.addTask("test-task", try json(fixture.allocator(), "{\"data\":\"test\"}"), null, null);
...
test "addTask: should add multiple tasks with unique IDs" {
    ...
    const id1 = try fixture.queue.addTask("test-task", try json(fixture.allocator(), "{\"data\":\"test1\"}"), null, null);
    const id2 = try fixture.queue.addTask("test-task", try json(fixture.allocator(), "{\"data\":\"test2\"}"), null, null);
    try std.testing.expect(!std.mem.eql(u8, id1, id2));
    try fixture.queue.awaitAllTasks();
}

test "addTask: hands the priority it was given to the backend" {
    var fixture: Fixture = undefined;
    try fixture.init(2);
    defer fixture.deinit();
    ...
    const testQueue = try TaskQueue.init(fixture.allocator(), std.testing.io, fixture.uuidGenerator.uuidGenerator(), "test");

    _ = try testQueue.addTask("test-task", try json(fixture.allocator(), "{\"data\":\"test\"}"), "task-id", .Interactive);

    try std.testing.expectEqual(@as(usize, 1), recordingBackend.capturedCount);
    try std.testing.expectEqualStrings("test-task", recordingBackend.capturedTypes[0]);
    try std.testing.expectEqualStrings("test", recordingBackend.capturedSources[0]);
    try std.testing.expectEqualStrings("task-id", recordingBackend.capturedIds[0]);
    try std.testing.expectEqual(@as(?TaskPriority, .Interactive), recordingBackend.capturedPriorities[0]);

    testQueue.deinit();
}

test "addTask: hands no priority to the backend when the caller named none" {
    var fixture: Fixture = undefined;
    try fixture.init(2);
    defer fixture.deinit();

    var recordingBackend: RecordingBackend = std.mem.zeroes(RecordingBackend);
    recordingBackend.capturedPriorities[0] = .Background;
    setQueueBackend(recordingBackend.queueBackend());
    const testQueue = try TaskQueue.init(fixture.allocator(), std.testing.io, fixture.uuidGenerator.uuidGenerator(), "test");

    _ = try testQueue.addTask("test-task", try json(fixture.allocator(), "{\"data\":\"test\"}"), "task-id", null);

    try std.testing.expectEqual(@as(usize, 1), recordingBackend.capturedCount);
    try std.testing.expectEqualStrings("task-id", recordingBackend.capturedIds[0]);
    try std.testing.expectEqual(@as(?TaskPriority, null), recordingBackend.capturedPriorities[0]);

    testQueue.deinit();
}

//
// The names of the "ordered-task" tasks in the order they ran.
//
...
//
// Released by the test once every task is queued, to let the first task finish.
//
var ordered_release: std.Io.Event = .unset;

//
// Records the task's name; the task named "running" holds its slot until the test releases it
// (Zig only: tasks run on threads, so the first task has to wait for the rest to be queued).
//
fn orderedTaskHandler(allocator: std.mem.Allocator, io: std.Io, data: std.json.Value, context: ITaskContext) anyerror!std.json.Value {
    _ = allocator;
    _ = context;
...
test "addTask: an interactive task is dispatched before background tasks already waiting" {
    var fixture: Fixture = undefined;
    try fixture.init(2);
    defer fixture.deinit();
    ordered_count.store(0, .release);
    ordered_release.reset();

    // A pool of one, so the first task holds the only slot and the rest queue behind it.
    const serialBackend = try MockWorkerPool.init(std.testing.io, 1, fixture.mockBackend.baseContext);
    setQueueBackend(serialBackend.queueBackend());
    const serialQueue = try TaskQueue.init(fixture.allocator(), std.testing.io, fixture.uuidGenerator.uuidGenerator(), "serial");

    try registerHandler("ordered-task", orderedTaskHandler);

    _ = try serialQueue.addTask("ordered-task", try json(fixture.allocator(), "{\"name\":\"running\"}"), null, null);
    _ = try serialQueue.addTask("ordered-task", try json(fixture.allocator(), "{\"name\":\"background-1\"}"), null, null);
    _ = try serialQueue.addTask("ordered-task", try json(fixture.allocator(), "{\"name\":\"background-2\"}"), null, null);
    _ = try serialQueue.addTask("ordered-task", try json(fixture.allocator(), "{\"name\":\"interactive\"}"), null, .Interactive);

    const pendingTypes = try serialBackend.getPendingTaskTypes(fixture.allocator());
    try std.testing.expectEqual(@as(usize, 3), pendingTypes.len);

    ordered_release.set(std.testing.io);
    try serialQueue.awaitAllTasks();
    serialQueue.deinit();
    setQueueBackend(fixture.mockBackend.queueBackend());
    serialBackend.deinit();

    const expected = [_][]const u8{ "running", "interactive", "background-1", "background-2" };
    try std.testing.expectEqual(expected.len, ordered_count.load(.acquire));
    for (expected, ordered_names) |expected_name, actual_name| {
...
test "awaitTask: awaitTask resolves when the specific task completes" {
    ...
    const taskId = try fixture.queue.addTask("test-task", .null, null, null);
...
test "awaitTask: multiple awaitTask callers on the same ID all resolve when the task completes" {
    ...
    const taskId = try fixture.queue.addTask("slow-task", .null, null, null);
...
test "awaitAllTasks: should wait for all tasks to complete" {
    ...
    _ = try fixture.queue.addTask("test-task", try json(fixture.allocator(), "{\"id\":1}"), null, null);
    _ = try fixture.queue.addTask("test-task", try json(fixture.allocator(), "{\"id\":2}"), null, null);
    _ = try fixture.queue.addTask("test-task", try json(fixture.allocator(), "{\"id\":3}"), null, null);
...
test "awaitAllTasks: should handle mixed success and failure" {
    ...
    _ = try fixture.queue.addTask("success-task", .null, null, null);
    _ = try fixture.queue.addTask("fail-task", .null, null, null);
    _ = try fixture.queue.addTask("success-task", .null, null, null);
...
test "awaitAllTasks: awaitAllTasks resolves immediately when shutdown is called while waiting" {
    ...
    _ = try fixture.queue.addTask("slow-task", .null, null, null);
...
test "parallel execution: should execute tasks in parallel up to maxWorkers" {
    ...
    while (task_number <= 5) : (task_number += 1) {
        ...
        _ = try fixture.queue.addTask("test-task", data, null, null);
    }
...
test "parallel execution: should respect maxWorkers limit" {
    ...
    while (task_number <= 10) : (task_number += 1) {
        const data = try json(fixture.allocator(), try std.fmt.allocPrint(fixture.allocator(), "{{\"id\":{d},\"sleep\":true}}", .{task_number}));
        _ = try queueWithLimit.addTask("test-task", data, null, null);
    }
...
test "parallel execution: should process tasks in FIFO order" {
    ...
    while (task_number <= 5) : (task_number += 1) {
        ...
        _ = try singleQueue.addTask("test-task", data, null, null);
    }
...
test "task data handling: should pass task data to handler" {
    ...
    _ = try fixture.queue.addTask("test-task", try json(fixture.allocator(), taskData), null, null);
    try fixture.queue.awaitAllTasks();
...
test "task data handling: should handle complex data structures" {
    ...
    _ = try fixture.queue.addTask("test-task", try json(fixture.allocator(), complexData), null, null);
    try fixture.queue.awaitAllTasks();
...
test "task data handling: should handle null data" {
    ...
    _ = try fixture.queue.addTask("test-task", .null, null, null);
    try fixture.queue.awaitAllTasks();
...
test "task data handling: should handle empty object data" {
    ...
    _ = try fixture.queue.addTask("test-task", try json(fixture.allocator(), "{}"), null, null);
    try fixture.queue.awaitAllTasks();
...
test "task data handling: should handle array data" {
    ...
    _ = try fixture.queue.addTask("test-task", try json(fixture.allocator(), arrayData), null, null);
    try fixture.queue.awaitAllTasks();
...
const RecordingBackend = struct {
    ...
    // The task types passed to addTask.
    capturedTypes: [4][]const u8,

    // The sources passed to addTask.
    capturedSources: [4][]const u8,

    // The priorities passed to addTask.
    capturedPriorities: [4]?TaskPriority,

    // Number of valid entries in capturedIds, capturedTypes, capturedSources and capturedPriorities.
    capturedCount: usize,
    ...
    // Records the task ID.
    //
    fn addTask(ptr: *anyopaque, allocator: std.mem.Allocator, io: std.Io, @"type": []const u8, data: std.json.Value, source: []const u8, taskId: ?[]const u8, priority: ?TaskPriority) anyerror![]const u8 {
        _ = allocator;
        _ = io;
        _ = data;
        const self: *RecordingBackend = @ptrCast(@alignCast(ptr));
        self.capturedIds[self.capturedCount] = taskId orelse "";
        self.capturedTypes[self.capturedCount] = @"type";
        self.capturedSources[self.capturedCount] = source;
        self.capturedPriorities[self.capturedCount] = priority;
...
test "addTask with explicit taskId: addTask with an explicit taskId passes that ID through to the backend" {
    ...
    const returnedId = try testQueue.addTask("test-task", .null, explicitId, null);
...
test "constructor options: should use custom UUID generator" {
    ...
    const taskId1 = try customQueue.addTask("test-task", .null, null, null);
    const taskId2 = try customQueue.addTask("test-task", .null, null, null);
...
test "constructor options: should use default UUID generator when not provided" {
    ...
    const taskId1 = try defaultQueue.addTask("test-task", .null, null, null);
    const taskId2 = try defaultQueue.addTask("test-task", .null, null, null);
...
test "error handling: should handle handler throwing non-Error objects" {
    ...
    _ = try fixture.queue.addTask("throw-string", .null, null, null);
    try fixture.queue.awaitAllTasks();
...
test "error handling: should preserve error messages" {
    ...
    _ = try fixture.queue.addTask("custom-error", .null, null, null);
    try fixture.queue.awaitAllTasks();
