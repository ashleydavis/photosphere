...
test "TaskContext: isCancelled returns false initially" {
    ...
    var context = TaskContext.init(uuid_generator.uuidGenerator(), timestamp_provider.timestampProvider(), "session-1", "task-1", recorder.sendMessageFn());
...
test "TaskContext: uuidGenerator, timestampProvider, sessionId, and taskId are exposed as provided" {
    var uuid_generator: utils.random_uuid_generator.RandomUuidGenerator = .{};
    var timestamp_provider: node_utils.test_timestamp_provider.TestTimestampProvider = .{};
    var recorder: MessageRecorder = .{ .calls = 0, .lastType = "", .lastValue = 0 };
    var context = TaskContext.init(uuid_generator.uuidGenerator(), timestamp_provider.timestampProvider(), "my-session", "my-task", recorder.sendMessageFn());
